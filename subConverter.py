...
class subConverter:
    ...
    def tagsBounds(self, line: str, tags: List[str], bounds: List[Any] = [], add: int = 0) -> Tuple[str, List[Any]]:
        ...
        while tag!=None:
            ...
            b = []
            if tag=='font':
                srch = re.compile(r'\s+color\s*=\s*[\'\"]{0,1}(\#[a-fA-F0-9]{6})[\'\"]{0,1}').search(line[min_pos:idx])
                if srch==None:
                    srch = re.compile(r'\s+size').search(line[min_pos:idx])
                    if srch==None:
    ...
    def readAss(self, fname_ass: str) -> List[Any]:
        ...
        with open(fname_ass) as fi:
            ...
            for line in fi:
                if block==2:
                    elems = line.split(',')
                    t =  re.compile(r'Style:\s*([^,]+)').match(elems[0])
                    if t:
                        sName = t.groups()[0]
                        t =  re.compile(r'\&(H[0-9a-fA-F]{2})([0-9a-fA-F]{6})').match(elems[3])
                        if t:
                            col = t.groups()[1]
                            styles[sName] = (col,)
                if line[:8] == 'alogue: ':
